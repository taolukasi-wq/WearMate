...
export default function AuthScreen({ onLogin }: AuthScreenProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      onLogin({
        name: mode === 'signup' ? fullName : 'Julian',
        email: email,
        ...
        <div className="w-full max-w-md">
          ...
          <div className="glass-card p-8 rounded-2xl shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-4">
              <button
                type="button"
                onClick={() => onLogin({ name: 'Julian', email: 'guest@apple.com', avatar: '', skinTone: 'Cool Ivory', bodyType: 'Hourglass' })}
              ...
              <button
                type="button"
                onClick={() => onLogin({ name: 'Julian', email: 'guest@google.com', avatar: '', skinTone: 'Cool Ivory', bodyType: 'Hourglass' })}
                className="flex items-center justify-center gap-2 py-3 border border-outline-variant/60 rounded-xl hover:bg-white transition-all active:scale-[0.98] cursor-pointer"
              >
                <svg className="w-5 h-5" viewBox="0 0 24 24">
                  <path
